...
const service = require("../services/PokerKingService");
...
async function OnWinNo() {
	...
	let winningSpot = result.spot;
	let data = { room_id: timeStamp, game_id:10,winNo1:winNo[0],winNo2:winNo[1],spot:winningSpot }
...
//---------------------------------------------------------------------------------------------
	    if (totalLeftBets === totalMiddleBets && totalMiddleBets === totalRightBets) {
	        ...
	        leastBetSpot = Math.floor(Math.random() * 54);//caculate random no form 0 to 53
	...
	function generateSpotWinningNo(leastBetSpot){
	    ...
	    let win2;
	     switch (leastBetSpot) {
	        case spot.left:
	            win1 = Math.floor( GetRandomNo(2,54) )       
	            win2 = Math.floor(GetRandomNo(1,win1))        
	            break;
	        case spot.middle:
	            win1 = Math.floor( GetRandomNo(1,54))        
	            win2 = win1      
	            break;
	        case spot.right:
	            win2 = Math.floor( GetRandomNo(2,54) )       
...
module.exports.StartPokerKing = StartPokerKing;
