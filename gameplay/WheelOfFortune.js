...
const service = require("../services/WheelOfFortuneService");
...
function StartWheelOfFortune(data){
...
async function OnWinNo() {
	...
	let winningSpot = result.spot;
	let data = { room_id: timeStamp, game_id:9,winNo1:winNo[0],winNo2:winNo[1],spot:winningSpot }
...
//---------------------------------------------------------------------------------------------
	    if (totalLeftBets === totalMiddleBets && totalMiddleBets === totalRightBets) {
	        ...
	        leastBetSpot = Math.floor(Math.random() * 21);//caculate random no form 0 to 20
	...
	function generateSpotWinningNo(leastBetSpot){
	    ...
	    let win2;
	     switch (leastBetSpot) {
	        case spot.left:
	            win1 = Math.floor( GetRandomNo(2,21) )       
	            win2 = Math.floor(GetRandomNo(1,win1))        
	            break;
	        case spot.middle:
	            win1 = Math.floor( GetRandomNo(1,21))        
	            win2 = win1      
	            break;
	        case spot.right:
	            win2 = Math.floor( GetRandomNo(2,21) )       
...
module.exports.StartWheelOfFortune = StartWheelOfFortune;
