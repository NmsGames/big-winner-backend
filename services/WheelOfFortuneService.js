...
const getRoundCount = async() => {
    try{
        let limit = 1;
        let sql =  `SELECT room_id FROM game_record_wheeloffortune ORDER BY id DESC LIMIT ?`
...
const lastWinningNo = async() => {
    try{
        ...
        let sql =  `SELECT spot FROM (SELECT * FROM game_record_wheeloffortune ORDER BY id DESC LIMIT ?) sub ORDER BY id ASC`
...
const updateWinningNo = async(data) => {
    try{
        let parms = data;
        let sql = "Insert Into game_record_wheeloffortune Set ?"
...
//         let arr = [];
//         let sql =  `SELECT spot FROM game_record_wheeloffortune ORDER BY id DESC LIMIT ?`
//         let data = await db.query(sql,limit);
//         if(isValidArray(data)){
...
const gameSlotRecords = async(data) => {
    try{
        ...
        let limit = 10;
        let result = new Array(limit);
        let sql =  `SELECT winNo1,winNo2 FROM (SELECT * FROM game_record_wheeloffortune ORDER BY id DESC LIMIT ?) sub ORDER BY id ASC`
        let data = await db.query(sql,limit);
        if(isValidArray(data)){
